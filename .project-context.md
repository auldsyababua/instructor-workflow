...
**Last Updated**: 2025-11-20 (V2 Agent Migration Complete)
...
**Security**:
- llm-guard>=0.3.0 - ML-based prompt injection detection (OWASP LLM01)
- PromptInjection scanner - DeBERTa-v3 transformer model for semantic attack detection
...
- Critical boundary statements prefixed with ⚠️
...
**ML-based Security Validation** (2025-01-14):
- **Problem**: Regex patterns cannot distinguish intent ("Implement bash runner" vs "Execute bash command")
- **Solution**: LLM Guard PromptInjection scanner uses semantic analysis, not keyword matching
- **Key Insight**: Attack scope separation critical - prompt injection (Layer 2) ≠ command injection (Layer 3)
- **Learning**: Test failures can validate correct architecture (attacks caught at right layer)
- **Pattern**: ML models provide semantic understanding but require clear architectural boundaries
- **Result**: 100% prompt injection detection with 0% false positives on benign development discussions
...
- **Solution**: 5-layer defense-in-depth validation architecture (input sanitization → Pydantic → semantic validation → rate limiting → audit logging)
...
**LLM Guard Integration** (2025-01-14):
- **Replaced**: Regex-based injection detection (13 patterns, keyword matching, high false positives)
- **Implemented**: LLM Guard ML-based semantic detection (DeBERTa-v3 transformer model)
- **Scope**: Layer 2 detects ONLY OWASP LLM01 (prompt injection) - NOT command injection or encoding operations
- **Performance**: 100% prompt injection detection (9/9 attacks), 100% false positive prevention (5/5 benign prompts)
- **Architectural Insight**: Command injection ("rm -rf", "sudo bash") belongs at Layer 3 (Capability Check), not Layer 2 (Injection Detection)
...
**DO NOT**:
- ❌ Use pattern matching for paths in hooks (use string containment: `'handoffs/' in path`)
- ❌ Use direct terminal spawning for agents (unreliable on Linux, use tmux)
- ❌ Assume per-agent MCP isolation works (not supported, use shared MCP with tool filtering)
- ❌ Expect relative paths in hooks (hooks receive absolute paths)
- ❌ Install npm with sudo (creates permission issues, use user directory)
- ❌ Use gnome-terminal for agent sessions (rendering issues, use kitty/alacritty)

**DO**:
- ✅ Use hooks for auto-deny with teaching feedback (validated working on PopOS 22.04)
- ✅ Use string containment for path matching in hooks (`'handoffs/' in path`)
- ✅ Launch with `--dangerously-skip-permissions` to skip prompts while enforcing via hooks
- ✅ Spawn agents via tmux sessions for reliable process isolation
- ✅ Use absolute paths in hook command configuration (settings.json)
- ✅ Set `chmod +x` on hook scripts before use
- ✅ Configure npm to user directory: `npm config set prefix '~/.npm-global'`
- ✅ Use kitty or alacritty terminal emulators on PopOS
- ✅ Parse hook context from stdin JSON, not environment variables
...
**Project Status**: Layer 5 security hardening COMPLETE - LLM Guard ML-based prompt injection detection operational

**Completed**:
1. ✅ Directory structure for IW agents created
2. ✅ Hook-based enforcement validated working on PopOS 22.04
3. ✅ auto-deny.py hook with teaching feedback implemented
4. ✅ Path matching fixed (string containment vs pattern matching)
5. ✅ Test violation files cleaned up
6. ✅ Instructor handoff validation (Pydantic models) implemented and documented
7. ✅ Layer 5 integration added to Planning Agent documentation
8. ✅ Observability dashboard deployed (Traefik + Bun + Vue) - http://workhorse.local/observability
9. ✅ Research completed: Instructor integration security hardening (1,520 lines)
...
2. 🔜 Grafana metrics integration (validation success rate, latency tracking)
3. 🔜 Rate limiting implementation (per-capability buckets)
4. 🔜 ValidatedAgentSpawner wrapper (fail-fast MVP with observability hooks)
...
**ADRs**:
- [ADR-001: Repository Organization](docs/architecture/adr/001-repository-organization.md)
- [ADR-002: V2 Agent Directory Migration](docs/architecture/adr/002-v2-agent-migration.md)

**Recent Changes**: V2 Agent Migration Complete - 26 canonical agents, action-agent deprecated, specialized implementation agents (frontend/backend/devops), 21 duplicate directories archived
...
│   ├── *-agent/               # 26 canonical agents (frontend, backend, devops, etc)
...
**Test Results**:
- ✅ Layer 3 (Hooks) FULLY FUNCTIONAL - contrary to GitHub issues suggesting unreliability
- ✅ PreToolUse hooks execute reliably with 100% success rate
- ✅ Exit code 2 blocks operations correctly
- ✅ Auto-deny.py provides helpful teaching feedback
- ✅ Path matching works with absolute paths using string containment
